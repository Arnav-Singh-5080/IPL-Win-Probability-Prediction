...
# -----------------------------------
st.set_page_config(
    page_title="CricScope", layout="wide", initial_sidebar_state="expanded"
)
...
def train_model():
    ...
    # Calibrated model
    pipe = CalibratedClassifierCV(estimator=base_model, method="sigmoid", cv=5)
    ...
    # Store metrics
    st.session_state.brier_score = round(brier, 4)
    st.session_state.log_loss = round(logloss, 4)
...
with st.sidebar:
    ...
    st.markdown(
        '<div style="height:1px; background:rgba(212,175,55,0.08); margin:16px 0;"></div>',
        unsafe_allow_html=True,
    )
    st.markdown(
...
if st.session_state.page == "Analysis":
    ...
    st.markdown(
        '<div style="height: clamp(16px, 4vw, 32px);"></div>', unsafe_allow_html=True
    )
    ...
    with col2:
        ...
        score = st.number_input(
            "Current Score", min_value=0, max_value=target - 1, value=50, step=1
        )
    ...
    st.markdown(
        '<div style="height: clamp(16px, 4vw, 28px);"></div>', unsafe_allow_html=True
    )
    ...
    with col1:
        st.metric("Calibrated Win Probability", f"{win * 100:.2f}%")

    with col2:
        st.metric("Reliability Level", reliability)

    with col3:
        st.metric("Brier Score", st.session_state.brier_score)

    st.caption(
        "Probabilities calibrated using sigmoid scaling "
        "with historical IPL match data."
    )
    ...
    # ---- SUMMARY ROW ----
    st.markdown(
        '<div style="height: clamp(8px, 2vw, 16px);"></div>', unsafe_allow_html=True
    )
